...
const projects: Project[] = [
  ...
  },
  {
    ...
    title: "BI 서비스 신규 구축",
    description:
      "약품 판매 실적 데이터를 제약사 운영자·사내 운영자에게 시각화·분석 형태로 제공하는 사내 BI 서비스. 제약사용 서비스 앱과 사내 어드민 콘솔 두 프로덕트를 pnpm 모노레포 한 저장소에서 공유 패키지 기반으로 구축",
    tags: [
      "React 19",
      "TypeScript",
      "Vite",
    ...
    period: "2026.04 - 진행중",
    role: "프론트엔드 개발 (아키텍처 설계·구현 주도)",
    company: "알스솔루션",
    highlights: [
      "제약사 사용자용 서비스 앱과 사내 운영자용 어드민 콘솔 두 프로덕트를 pnpm workspaces 모노레포로 구성. 인증·차트·계약사/계정 도메인·공통 컴포넌트를 shared 패키지로 공유하되, 앱 셸(라우터·가드·경로)은 각 앱이 독립 보유해 앱 간 교차 의존 제거",
      "서비스 앱: 대시보드 · 통합분석(출고·채널·지역·시장·제품 5축) · 데이터 활용 · 계정관리 · 고객센터(FAQ·공지사항·1:1 문의). 어드민 앱: 대시보드 · 계약사 관리 · 어드민 계정 관리 · 운영 콘텐츠 관리 · 데이터 전처리 관리 · 보안/접근 로그",
      "직전 사내 백오피스 리팩토링에서 사전 검증한 Antd 디자인 시스템을 본격 적용. 검증 단계에서 정의한 토큰 매핑·컴포넌트 패턴을 그대로 재사용해 화면 셋업 비용 절감",
      "차트를 디자인 시스템의 일부로 편입: 차트 라이브러리(@ant-design/plots·@antv) 직접 import를 금지하고 shared 래퍼 6종(Line·Column·Bar·Area·Pie·ComboTrend)만 공개. 진입점은 동적 import 단일 지점으로 통일해 차트 번들이 초기 로드에 포함되지 않도록 분리했고, 팔레트·높이·다크모드 대응을 토큰 레이어에 집중",
      "단일 y축 막대+라인 혼합 차트에서 G2 DualAxes가 자식별로 y 스케일을 독립 강제해 라인이 소실되는 문제 확인 → interval·line 마크가 y·color 스케일을 공유하도록 G2 view를 직접 조립한 ComboTrendChart를 자체 구현해 대체",
      "아키텍처 규약을 도구로 강제: eslint-plugin-boundaries로 app → features → shared 단방향 import와 feature 간 직접 참조 금지를 lint 에러로 고정하고, 백엔드 SDK import를 각 기능의 데이터 레이어(types·api·queries 3층)로만 제한하는 경계 규칙을 테스트로 검증",
      "React Query의 AbortSignal을 두 앱·shared의 조회 함수 65개 전 구간에 관통 전달해 화면 이탈·필터 재조회 시 실제 HTTP 요청이 취소되도록 구성. 시그널 누락을 잡는 구조 검사 테스트와 런타임 취소 회귀 테스트를 함께 추가",
      "인증 원본을 서버 HttpOnly refresh 쿠키로 두고, 클라이언트 저장소는 라우팅·표시용 캐시로만 한정. 앱 시작 시 로컬 캐시 유무와 무관하게 /me로 서버 세션을 동기화하고, 401/403처럼 서버가 인증 실패를 확정한 경우에만 로그아웃 — 네트워크 오류·5xx는 로그인 화면으로 튕기지 않고 재시도 상태를 유지하도록 계약 문서화",
      "역할 4종(알스마스터·알스어드민·계약사마스터·계약사일반) RBAC 적용. 권한 없는 작업은 버튼·메뉴를 애초에 렌더하지 않고, 직접 URL 진입은 라우트 가드로 2차 차단하는 이중 방어 구조로 설계",
      "라우트 단위 코드 스플리팅 적용. React.lazy가 실패한 프라미스를 payload에 확정 저장해 에러 바운더리를 리셋해도 복구되지 않는 문제를 세대(generation) 기반 재시도 래퍼로 해소하고, 브라우저 module map 캐시 한계상 같은 URL 재요청은 무의미하다는 점을 확인해 자동 재시도 대신 새로고침 유도로 설계",
      "클라이언트 오류 수집 파이프라인 구축: 전체 queryKey 대신 식별자를 제거한 정적 작업명만 전송하도록 전송 정책을 두고, 수집기 연결을 런타임 브리지로 분리해 앱 번들 재배포 없이 벤더 교체가 가능한 구조로 구성",

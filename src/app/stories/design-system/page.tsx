...
const stages = [
  {
    ...
    why: "백오피스 도메인 비중이 높은 사내 환경에 맞춰 컴포넌트 풍부도·디자인 커스터마이징 자유도·반응형 그리드 성숙도가 균형 잡힌 Bootstrap 채택. 사내 정체성 디자인은 CSS Variables로 추상화하여 Bootstrap 위에 일관 적용",
    nextHook:
      "이 시스템이 운영 환경에서 어떻게 동작하는지 어디서 검증할 수 있을까? → 마침 시작 단계인 사내 백오피스 신규 구축이 첫 적용 환경으로 적합",
  },
  {
    period: "2025.05 - 06",
    slug: "backoffice",
    ...
    why: "공지·약관·FAQ·회원·파일 업로드 등 운영 도메인의 다양한 화면 패턴(폼·테이블·검색·첨부)에 토큰·컴포넌트 구조가 어떻게 동작하는지 검증",
    nextHook:
      "운영 약 1년 시점, 사내 차기 신규 프로젝트(BI 서비스)에서 Antd 도입 결정. 신규 프로젝트에 바로 던지지 않고, 운영 중인 백오피스에서 먼저 검증하기로 결정",
  },
  {
    period: "2026.02 - 03",
    ...
    why: "신규 BI 프로젝트에 Antd를 바로 적용하면 라이브러리 부적합·디자인 정체성 적용 한계 등의 리스크가 운영 단계에서 발견될 수 있음. 운영 중 백오피스에서 먼저 마이그레이션 진행하여 Antd 기반 워크플로우·디자인 적용 가능성·운영 리스크를 사전 검증",
    nextHook:
      "검증 단계에서 정의한 토큰 매핑·컴포넌트 사용 패턴이 신규 BI 프로젝트의 초기 개발 속도를 결정",
  },
  {
    period: "2026.04 - 진행중",
    slug: "bi-service",
    ...
    why: "직전 백오피스 리팩토링에서 사전 검증한 토큰 매핑·컴포넌트 사용 패턴을 그대로 활용. 차트 라이브러리도 디자인 시스템과의 시각적 일관성을 위해 같은 계열인 Antd Charts 채택",
    nextHook: null,
  },
];
    ...
    <Section>
      ...
      <div className="mt-12 rounded-lg border border-border bg-muted/30 p-6">
        ...
        <p className="leading-relaxed text-muted-foreground">
          새 라이브러리를 신규 프로젝트에 바로 던지지 않고, 운영 중인 환경에서
          먼저 검증한 뒤 본격 적용한다. 디자인 시스템은 만들고 끝나는 결과물이
          아니라 <strong className="text-foreground">운영 → 학습 → 진화</strong>의
          사이클을 거치는 살아있는 시스템이라는 관점.
        </p>
      </div>
      ...
      <div className="mt-16 rounded-lg border border-accent/30 bg-accent/5 p-6">
        ...
        <ul className="mt-4 space-y-3 leading-relaxed text-muted-foreground">
          <li>
            ・ 1단계 디자인 시스템 구축 시점에 Antd/MUI 같은 큰 시스템과의 비교
            검토를 더 명시적으로 문서화했더라면, 이후 3단계 마이그레이션 의사결정
            근거가 더 단단했을 것
          </li>
          <li>
            ・ 2단계 백오피스 첫 적용에서 더 다양한 화면 패턴(데이터
            시각화·복합 폼)까지 검증했다면 3단계 Antd 결정 시점이 앞당겨졌을
            가능성
          </li>
          <li>
            ・ 디자인 시스템의 ‘운영 → 학습 → 진화’ 사이클을 더 짧게 가져가기
            위해, 다음부터는 분기별 회고를 명시적인 단계로 두는 방향
          </li>
        </ul>
      </div>
    </Section>

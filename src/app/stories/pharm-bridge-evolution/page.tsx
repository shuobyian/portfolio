...
const stages: Stage[] = [
  {
    ...
    why: "백엔드 Spring 전환과 동기화된 일정 안에 안정화해야 하는 제약이 가장 컸습니다. 여기에 SSR 라우팅(서버 렌더된 페이지 셸을 그대로 활용해야 하는 제약)·운영 인력의 장기 유지보수 가능 기술 폭을 함께 고려한 결과, React를 도입하면 구조 설계·학습 비용 때문에 전환 일정 내 안정화가 어렵다고 판단했습니다.",
    nextHook:
  ...
  },
  {
    ...
    nextHook:
      "구조 개선의 효과는 체감으로만 말하지 않고, 도입 전후 번들 사이즈를 직접 실측해 트레이드오프를 숫자로 확인하고 싶었습니다.",
  },
  {
    period: "2026.03",
    slug: "pharm-bridge-refactor",
    title: "5. 번들 사이즈 실측으로 트레이드오프 검증",
    subtitle: "체감이 아니라 숫자로",
    decision:
      "리팩토링 전후 번들·에셋 사이즈를 직접 측정해 React 도입의 성능 영향을 정량 확인",
    why: "SearchWait 페이지를 기준으로 gzip 전송량을 비교했습니다. React 도입으로 jQuery 등 외부 CDN 의존(약 207 KB)은 사라졌지만, 단일 페이지 첫 진입 비용은 SSR 약 111 KB → React 약 150 KB(gzip)로 오히려 늘었습니다. 단일 페이지만 보면 손해처럼 보이지만, 코드 스플리팅 구조라 사이트 전체·재방문 캐시까지 보면 분할 구조가 역전 우위를 가질 여지가 있다고 분석했습니다. ‘React가 무조건 가볍다’가 아니라, 측정해 보니 트레이드오프였고 그 트레이드오프를 어디서 회수하는지를 이해하는 것이 핵심이었습니다.",
    nextHook: null,
  },
];
